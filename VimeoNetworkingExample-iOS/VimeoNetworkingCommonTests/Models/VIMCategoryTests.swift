...
class VIMCategoryTests: XCTestCase {
    ...
    func testCategoryModelVersion() {
        let version = VIMCategory.modelVersion()
    ...
    func testKeyValueDictionary() {
        let dict = category.keyValueDictionary()
        assertSnapshot(matching: dict, as: .dump)
    }
